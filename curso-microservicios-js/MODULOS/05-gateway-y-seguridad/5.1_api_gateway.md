...
📁 **Ejemplo mínimo en Express:** [EJEMPLOS/gateway_minimo](../../EJEMPLOS/gateway_minimo) — enruta `/api/users` y `/api/orders` hacia los puertos de [EJEMPLOS/distribuida](../../EJEMPLOS/distribuida) (3001 y 3002). El laboratorio **5.4** añade JWT y `auth-service`.
...
implementaremos un **API Gateway en Express con autenticación JWT** (`axios` + middleware propio),
donde el **auth-service** emite el token y el gateway y **orders-service** validan el JWT.
...
[← 4.3 Práctica CQRS](../04-cqrs-y-event-sourcing/4.3_practica_cqrs.md) · [Temario](../../../README.md) · [Siguiente: 5.2 Seguridad distribuida →](5.2_seguridad_distribuida.md)